...
object FilterByDateModule {
  ...
  implicit class FilterByDateTimeSeriesOperations[V](ts: TimeSeries[V]) {
    ...
    private def getFirstOf(unit: ChronoUnit): TimeSeries[V] = {
      ts.groupByTime(_.truncatedTo(unit), _.sortBy(_._1).unzip._2.head)
    }

    def getFirstOfTheMinute: TimeSeries[V] = getFirstOf(ChronoUnit.MINUTES)

    def getFirstOfTheHour: TimeSeries[V] = getFirstOf(ChronoUnit.HOURS)

    def getFirstOfTheDay: TimeSeries[V] = getFirstOf(ChronoUnit.DAYS)

    def getFirstOfTheMonth(zoneOffset: ZoneOffset = ZoneOffset.UTC): TimeSeries[V] = {
      ...
      ts.groupByTime(truncateMonth, _.sortBy(_._1).unzip._2.head)
    }

    def getFirstOfTheYear(zoneOffset: ZoneOffset = ZoneOffset.UTC): TimeSeries[V] = {
      def truncateMonth(i: Instant): Instant = {
        LocalDateTime.ofInstant(i, zoneOffset)
          .withMonth(1)
          .withDayOfMonth(1)
