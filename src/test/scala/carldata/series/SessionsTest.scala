...
class SessionsTest extends FlatSpec with Matchers {
  ...
  val seriesWithTimestampGaps = Csv.fromString(
    """time,value
      |2008-01-01T12:35:00, 4
      |2008-01-01T12:36:00, 2
      |2008-01-01T12:37:00, 3
  ...
  it should "detect sessions with toleration of 4 minutes in typical rain signal" in {
    val expected = Seq(Session(0, 17), Session(23, 29))
    Sessions.findSessions(seriesOfTypicalRain, Duration.ofMinutes(4)) shouldBe expected
  }

  it should "detect sessions with toleration of 1 minute in signal having timestamp gaps" in {
    val expected = Seq(Session(0, 2), Session(7, 7), Session(9, 15), Session(20, 21), Session(24, 25))
    Sessions.findSessions(seriesWithTimestampGaps, Duration.ofMinutes(1)) shouldBe expected
  }

  it should "detect sessions with toleration of 3 minutes in signal having timestamp gaps" in {
    val expected = Seq(Session(0, 2), Session(7, 15), Session(20, 25))
    Sessions.findSessions(seriesWithTimestampGaps, Duration.ofMinutes(3)) shouldBe expected
  }

  it should "detect sessions with toleration of 4 minutes in signal having timestamp gaps" in {
    val expected = Seq(Session(0, 15), Session(20, 25))
    Sessions.findSessions(seriesWithTimestampGaps, Duration.ofMinutes(4)) shouldBe expected
  }
